...
void LaserSlamWorker::init(
 ...
 std::ofstream record0("/home/hasee/segmap_ws/data/path0.txt", std::ios::trunc);
 std::ofstream record1("/home/hasee/segmap_ws/data/path1.txt", std::ios::trunc);
 std::ofstream recordtime("/home/hasee/segmap_ws/data/time.txt", std::ios::trunc);
int pathNum = 1;
void LaserSlamWorker::scanCallback(const sensor_msgs::PointCloud2& cloud_msg_in) {
  ...
  if (!lock_scan_callback_) {
    if (tf_listener_.waitForTransform(params_.odom_frame, params_.sensor_frame,
     ...
     record1<<pathNum<<" "<<current_pose.getPosition()[0]<<" "<<current_pose.getPosition()[1]<<" "<<current_pose.getPosition()[2]<<std::endl;
      pathNum++;
      if (process_scan) {
        ...
        recordtime<<"cloud: "<<cloud_msg_in.header.stamp.toNSec()<<" scan: "<<new_scan.time_ns<<std::endl;
        ...
        if (params_.remove_ground_from_local_map) {
          const double robot_height_m = current_pose.T_w.getPosition()(1);
          PointCloud new_fixed_cloud_no_ground;
          for (size_t i = 0u; i < new_fixed_cloud_pcl.size(); ++i) {
            if (new_fixed_cloud_pcl.points[i].y > robot_height_m -
...
//   pose.time_ns = rosTimeToCurveTime(tf_transform.stamp_.toSec());
  return pose;
}

Time LaserSlamWorker::rosTimeToCurveTime(const Time& timestamp_ns) {
 // std::cout<<"before: "<<timestamp_ns<<std::endl;
  if (!base_time_set_) {
    base_time_ns_ = timestamp_ns;
    base_time_set_ = true;
  }
 //  std::cout<<"after: "<<timestamp_ns<<" "<<base_time_ns_<<std::endl;
